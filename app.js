...
// --- Gyroskop Licht-Effekt (Device Orientation) ---
function handleOrientation(event) {
    let x = event.gamma; // Links/Rechts Neigung [-90 bis 90]
    let y = event.beta;  // Vor/Zurück Neigung [-180 bis 180]

    // Begrenzen der Neigung auf einen sinnvollen Radius (max 45 Grad)
    if (x > 45) x = 45;
    if (x < -45) x = -45;
    if (y > 45) y = 45;
    if (y < -45) y = -45;

    // Umrechnen der Gradzahlen in Prozent (0% bis 100%)
    let xPercent = ((x + 45) / 90) * 100;
    let yPercent = ((y + 45) / 90) * 100;

    // CSS Variablen global aktualisieren
    document.documentElement.style.setProperty('--light-x', `${xPercent}%`);
    document.documentElement.style.setProperty('--light-y', `${yPercent}%`);
}

// Sensoren standardmäßig aktivieren (funktioniert direkt auf Android)
window.addEventListener('deviceorientation', handleOrientation);

// iOS 13+ Permission Fix: Fordert Erlaubnis an, sobald der Nutzer das erste Mal tippt
let orientationPermissionGranted = false;
document.body.addEventListener('click', () => {
    if (!orientationPermissionGranted && typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
        DeviceOrientationEvent.requestPermission()
            .then(permissionState => {
                if (permissionState === 'granted') {
                    orientationPermissionGranted = true;
                    // Neu binden nach erfolgreicher Erlaubnis
                    window.addEventListener('deviceorientation', handleOrientation);
                }
            })
            .catch(console.error); // Ignoriert Fehler stumm, falls es abgelehnt wird
    }
}, { once: true }); // Führt diesen Check nur ein einziges Mal beim ersten Klick aus
